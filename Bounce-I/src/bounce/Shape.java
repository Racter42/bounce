...
public abstract class Shape {
   ...
   protected int nestDeltaX = 0;
	
   protected int nestDeltaY = 0;
...
/**
	 * Creates a Shape instance with specified x, y, deltaX, deltaY, and text values.
	 * The Shape object is created with a default width and height.
	 */
   ...
   public void paint(Painter painter) {
      ...
      if (_text != "") {
         int fWidth, fHeight;
         fWidth = _width / 2 + _x;
         fHeight = _height / 2 + _y;
         painter.drawCenteredText(_text, fWidth, fHeight);
      }
   }
   protected abstract void doPaint(Painter p);
   ...
   public void setParent(NestingShape shape) {
      _parent = shape;
      this.nestDeltaX = shape.deltaX() + shape.nestDeltaX;
      this.nestDeltaY = shape.deltaY() + shape.nestDeltaY;
   }
	
   public List<Shape> path() {
      List<Shape> orderedPath = null;
      if (_parent != null) {
         orderedPath = _parent.path();
         orderedPath.add(this);
      }
      else {
         orderedPath = new ArrayList<Shape>();
         orderedPath.add( this );
      }
